...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--disable-histgradient",
        action="store_true",
        help="Skip HistGradientBoosting base learner (UID157 ckmon 6th member).",
    )
    parser.add_argument(
        "--sequence-only",
        action="store_true",
...
def _make_base_models(
    *,
    ...
    if enable_histgradient:
        # UID157 ckmon 6th member (from their published artifact params).
        models.append(
            (
                "histgradient",
                HistGradientBoostingClassifier(
                    max_iter=int(os.getenv("HGB_MAX_ITER", "600")),
                    learning_rate=0.05,
                    max_leaf_nodes=63,
                    min_samples_leaf=20,
...
def train(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    base_specs = _make_base_models(
        ...
        enable_histgradient=(not args.disable_histgradient) and (not args.sequence_only),
