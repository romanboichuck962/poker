    ...
    def __init__(self, config=None):
        ...
        self.model_manifest = build_local_model_manifest(
            ...
            defaults={
                ...
                "notes": "uid242 v8: switched back from UID142's 'cold' stacked model to uid77's UID163 rocket architecture (rocket_logit), retrained in this repo on the public benchmark through 2026-07-21 (57 releases, 2936 balanced chunks) with live-OOD ablation from uid242's own 720 captured validator chunks, then blend weights re-controlled via a dense 946-point walk-forward simplex search on OUR reward() with per-candidate FPR-anchored thresholds. Selected weights stack 0.20/mono 0.14/mlp 0.42/drse 0.24; walk-forward reward 0.9086, AP 0.9429, recall@FPR<=0.05 0.762, hard_fpr 0, safety 1.0. Force-deployed past the promote gate (the stored baseline 0.9247 was measured on an earlier, easier window ending 07-19; this candidate is scored on the harder window including the two newest dates). Serving uses the rocket's rank-preserving remap + 12.5% batch positive cap.",
            },
        )
